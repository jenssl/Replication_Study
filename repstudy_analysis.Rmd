...
chisq_df1 <- data1 %>%
  select (condition, differencetotal_pos) %>%
  filter (differencetotal_pos >= 1) %>%
  mutate(condition = recode(condition, rewardgoal = 1, meregoal=2, doyourbest= 3)) %>%
  select (condition, differencetotal_pos)
  
chisq <- chisq.test(chisq_df1)
print(chisq)
...
#for the cases in which 7 or 8 words were created, 
df4 <- data1 %>%
  select (diff_1:diff_7, condition, differencetotal_pos, actual_1:actual_7) 


  filter (actual_1==8 & actual_1==7 | actual_2==8 & actual_2==7| actual_3==7 & actual_3==8 | actual_4==8 & actual_4==7 | actual_5==8 & actual_5==7 | actual_6==7 & actual_6==8 | actual_7==8 & actual_7==7)

df4 <- df4[df4$actual_1 %in% c(7, 8), ]
  
```

Goal commitment scores
