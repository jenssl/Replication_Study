...
df <- read_csv("methods/Sarah's Replication Study (10.28.21).csv")
View(df)

```
Clean the data
```{r, include=FALSE}
##remove rows 2 & 3 which are additional headers from Qualtrics. Column names are now in row 1
##remove the first 16 columns that include the start/end dates, language, etc. 
## select all the columns I need for analysis (mostly take out timing questions)
df <- df %>%
  filter (comp1 == "2m3v") %>%
  select (comp1:actual_7) %>%
  select (comp1, comp2, practice1, practice2, r1, r2, r3, r4, r5, r6, r7, goalcommit_1, goalcommit_2, goalcommit_3, goalcommit_4, goalcommit_5, goalcommit_6, goalcommit_7, goalcommit_8, report_1, report_2, report_3, report_4, report_5, report_6, report_7, bonus_report_reward, goal_report_mere, report_dyb, age, gender, race, work_exp, sanitycheck, anythingodd, condition, actual_1, actual_2, actual_3, actual_4, actual_5, actual_6, actual_7) #should have 43 columns
...
mean(data1$age, na.rm = TRUE) #36.69
sd(data1$age, na.rm = TRUE) #13.44
table (data1$gender) #51% female (86/169)
table (data1$race) #76% white (129/169)
data1$work_exp <- as.numeric(data1$work_exp)
mean(data1$work_exp, na.rm = TRUE) #14.86
...
t.test(actualtotal ~ condition, ttest_df, var.equal = TRUE)

#t-test comparing reported number of words
t.test(reporttotal ~ condition, ttest_df, var.equal = TRUE)

#t-test comparing difference scores
t.test(differencetotal ~ condition, ttest_df, var.equal=TRUE)
...
plot1 <- data1 %>%
  group_by (condition) %>%
  summarize (mean_report=mean(reporttotal)) %>%
  ggplot (aes(condition, mean_report, fill=condition)) +
...
table427 <- data1 %>%
  select (condition, actual_1:acutal_7) %>% 
  group_by (condition) %>% 
  get_summary_stats (reporttotal, actualtotal, differencetotal, type="mean_sd")
